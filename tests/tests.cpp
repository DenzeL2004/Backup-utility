...
TEST(NoExceptions, PARSER) {
    ASSERT_NO_THROW(ParseArguments(argc, argv_full));
    ASSERT_NO_THROW(ParseArguments(argc, argv_inc));
}

TEST(UncorrectArgc, PARSER) {
    ASSERT_ANY_THROW(ParseArguments(1, argv_full));
}

TEST(UndefineCommand, PARSER) {
    char* argv[] = {"rubish", "undefinwe", work_dir_name.data(), backup_dir_name.data()};
    ASSERT_ANY_THROW(ParseArguments(argc, argv));
}
